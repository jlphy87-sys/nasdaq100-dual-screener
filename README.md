...
| D20 | 관심 탭 가상 매매 표식: 매수/매도 마킹 시 그날 시세·날짜 자동 기록(일봉 기반 — 장중 체결가 없음, WebView 는 prompt 미지원), 보유중 평가손익·매도 후 확정손익 표시. 사이클 1개만 보관(다시 매수 시 덮어씀). 탈출구: 이력 필요 시 trades[] 배열로 확장 |
