...
| D20 | 관심 탭 가상 매매 표식: 매수/매도 버튼 → 인라인 가격 입력폼(기본값=그날 종가, 사용자가 체결가로 조절 가능; WebView 는 prompt 미지원이라 인라인 폼). 보유중 평가손익·매도 후 확정손익 표시, 봉차트에 매수/매도 가격 점선 기준선+라벨(범위 밖이면 스케일 확장; 봉 날짜 배열이 없어 X 위치 마킹은 불가 — 가격선이 정직한 근사). 다시 매수 시 완결 사이클은 trades[] 이력으로 자동 보존(종목당 50건 상한), 관심 탭 상단에 매매 기록 표(종목·매수·매도·손익, 확정 건수·승수·평균 요약) + 행별/카드별 기록 삭제(✕) |
